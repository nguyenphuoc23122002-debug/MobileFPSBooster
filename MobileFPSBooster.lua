...
-- Script này require DebugSystem từ ReplicatedStorage
...
if ReplicatedStorage:FindFirstChild("DebugSystem") then
    ...
    -- Nếu chưa có module, fallback nhẹ để tránh lỗi khi copy nhanh
...
-- AUTO FIX LAG (Chạy ngay khi load script) - wrapped by DebugSystem.try
...
local function CheckWall(targetPart)
    ...
    local ok, result = pcall(function()
        ...
        return workspace:Raycast(rayOrigin, rayDirection.Unit * rayDirection.Magnitude, rayParams)
    end)
    if ok and result and result.Instance and not result.Instance:IsDescendantOf(targetPart.Parent) then
...
DebugSystem.info("✅ Fix Lag: AUTO (Luôn bật)", "SYSTEM")
DebugSystem.info("🎯 Aimbot: Bấm ON để kích hoạt", "SYSTEM")
